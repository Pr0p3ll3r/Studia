...
function Formularz({ dodajPrace }) {
    ...
    const [piorytet, ustawPiorytet] = useState('')
    const handleSubmit = (e) => {
        dodajPrace([opis, nazwa, data, piorytet])
        e.preventDefault()
    }
    return (
        <form onSubmit={handleSubmit} class="col-lg-6 offset-lg-3">
            ...
            <input type="checkbox" name="piorytet"
                value={piorytet} onChange={e => {
                    if(e.target.checked) ustawPiorytet("TAK")
                    else ustawPiorytet("NIE")
                }
                }/> Piorytet<br /><br />
